...
while True:
    ret, frame = cap.read()
    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV) # HSV변환
    lowest = np.array([0, 48, 80], dtype = "uint8") #피부색 저점 범위 설정
    highest = np.array([20, 255, 255], dtype = "uint8") #피부색 고점 범위 설정
    skinHSV = cv2.inRange(hsv, lowest, highest) #피부색 범위 저장
    blurring = cv2.GaussianBlur(skinHSV, (7, 7), 3) #흐릿하게 하기위한 블러링
